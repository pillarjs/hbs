...
describe('express 4.x async helpers', () => {
  ...
  test('index', async () => {
    const expected = fs.readFileSync(path.join(FIXTURES_DIR, 'async.html'), 'utf8')
    await request(app).get('/').expect(expected)
  })

  test('async', async () => {
    const expected = fs.readFileSync(path.join(FIXTURES_DIR, 'fake-async.html'), 'utf8')
  ...
  test('async-with-params', async () => {
    const expected = fs.readFileSync(path.join(FIXTURES_DIR, 'async-with-params.html'), 'utf8')
    await request(app).get('/async-with-params').expect(expected)
  })

  test('async-with-layout', async () => {
    const expected = fs.readFileSync(path.join(FIXTURES_DIR, 'async-with-layout.html'), 'utf8')
    await request(app).get('/async-with-layout').expect(expected)
  })

  test('layout-with-async', async () => {
    const expected = fs.readFileSync(path.join(FIXTURES_DIR, 'layout-with-async.html'), 'utf8')
